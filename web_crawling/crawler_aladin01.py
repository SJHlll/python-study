...
# 크롬 드라이버를 별도로 설치하지 않고 버전에 맞는 드라이벌르 사용하게 해주는 코드
service = webdriver.ChromeService(ChromeDriverManager().install())

# 크롬 드랑비러를 활용하여 웹 브라우저를 제어할 수 있는 객체를 리턴
...
이름을 적으면 해당 태그만 전부 추추랗여 리스트에 담아 대입한다
'''
...
# li 안에 우리가 필요로 하는 텍스트가 존재함
# 그 중 2, 3, 4번 li의 텍스트를 가져와야 함
